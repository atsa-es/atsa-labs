...
For the process model, there are a number of ways to parameterize the first state ($x_1$). Because $x$ is a random walk model not a stationary AR(1), we will place a vague weakly informative prior on $x_0$: $x_0 \sim \,\text{N}(y_1, \sigma=10)$. Note, because we use $Y_1$, our first data point should not be NA. `EY` is added so that we can track the model fits for $y$. In this case it is just `X` but in more complex models it will involve more parameters.
