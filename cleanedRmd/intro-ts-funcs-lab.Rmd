```{r ts-setup-lab, include=FALSE, purl=FALSE}
knitr::opts_knit$set(unnamed.chunk.label = "tslab-")
```

# Basic time series functions in R  {#chap-ts-lab}
...
The chapter uses two data sets described in Section \@ref(sec-ts-data).  The main one is a CO$_2$ data set: [CO2_data.RData](./data/CO2_data.RData). The second is   a Northern Hemisphere temperature data set:  [NHemiTemp_data.RData](./data/NHemiTemp_data.RData).  The problems use a data set on hourly phytoplankton counts: [hourly_phyto.RData](./data/hourly_phyto.RData). After downloading via the links, load the data
```{r ts-load-datasets, eval=FALSE}
load("CO2_data.RData")
...
```{r ts-loadpackages, warning=FALSE, message=FALSE, results='hide'}
library(stats)
library(MARSS)
library(forecast)
library(datasets)
...
## CO$_2$ and global temperature data set {#sec-ts-data}

For this chapter we will use the time series of the atmospheric concentration of CO$_2$ collected at the Mauna Loa Observatory in Hawai'i.  We downloaded this data and created the `CO2` dataframe with the following code:
```{r ts-CO2data, eval=FALSE}
library(RCurl)
...
```
The data file contains some extra information that we don't need, so we only save a subset of the data, specifically columns ( 1, 2 \& 5), and save this to `CO2`. The object `CO2fulltext` contains information on the data.  Use `cat(CO2fulltext)` to read.  You can read in the RData file here: [CO2_data.RData](./data/CO2_data.RData). 

```{r ts-copy-data-file-to-doc, include=FALSE, purl=FALSE}
#If bookdown is being run, then we are at the top level
#make sure the Rdata is in the docs folder
#file.copy("Lab-intro-to-ts/CO2_data.RData", "docs/data/CO2_data.RData", overwrite=TRUE)
...
if("bookdown::gitbook" %in% rmarkdown::all_output_formats('index.Rmd')){ 
  #load the data
  load("docs/data/CO2_data.RData")
}else{
  load("CO2_data.RData")
}
```

The second data set we will use is Northern Hemisphere land and ocean temperature anomalies from NOAA. You can read in the RData file here:  [NHemiTemp_data.RData](./data/NHemiTemp_data.RData).  We downloaded these data and created the RData file with the following code:
```{r ts-temp-data, eval=FALSE}
library(RCurl)
...
```{r ts-copy-temp-data-to-doc, include=FALSE, purl=FALSE}
#If bookdown is being run, then we are at the top level
#make sure the Rdata is in the docs folder
#file.copy("Lab-intro-to-ts/NHemiTemp_data.RData", "docs/data/NHemiTemp_data.RData", overwrite=TRUE)
#If "gitbook" is not in the output
#then user is running Rmd in the folder for debugging
if("bookdown::gitbook" %in% rmarkdown::all_output_formats('index.Rmd')){ 
  #load the data
  load("docs/data/NHemiTemp_data.RData")
}else{
  load("NHemiTemp_data.RData")
}
```

Once you have downloaded the RData files (or created your own with the code), load the data files with:
```{r eval=FALSE}
load("CO2_data.RData")
load("NHemiTemp_data.RData")
```

## Time series plots {#sec-ts-time-series-plots}
...
```{r ts-CO2ts, echo=TRUE, eval=TRUE}
...
```{r ts-plotdataPar1, eval=FALSE, echo=TRUE}
## plot the ts
plot.ts(co2, ylab=expression(paste("CO"[2]," (ppm)")))
```


(ref:ts-plotdata1) Time series of the atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i measured monthly from March 1958 to present.

```{r ts-plotdata1, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotdata1)'}
...
Examination of the plotted time series (Figure \@ref(fig:ts-plotdata1)) shows 2 obvious features that would violate any assumption of stationarity: 1) an increasing (and perhaps non-linear) trend over time, and 2) strong seasonal patterns. (*Aside*: Do you know the causes of these 2 phenomena?)


### Combining and plotting multiple **ts** objects {#sec-ts-combining-and-plotting-multiple-textbfts-objects}

Before we examine the CO$_2$ data further, however, let's see a quick example of how you can combine and plot multiple time series together. We'll use the [data](#sec-ts-data) on monthly mean temperature anomolies for the Northern Hemisphere (``Temp``).  First convert ``Temp`` to a ``ts`` object.
...
```{r ts-alignData, echo=TRUE, eval=TRUE}
...
It turns out that the regular ```plot()``` function in R is smart enough to recognize a **ts** object and use the information contained therein appropriately.  Here's how to plot the intersection of the two time series together with the y-axes on alternate sides (results are shown in Figure \@ref(fig:ts-plotdata2)):

```{r ts-plotdataPar2, eval=FALSE, echo=TRUE, fig.show='hide'}
## plot the ts
plot(datI, main="", yax.flip=TRUE)
```


(ref:ts-plotdata2) Time series of the atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i (top) and the mean temperature index for the Northern Hemisphere (bottom) measured monthly from March 1958 to present.

```{r ts-plotdata2, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=5, fig.cap='(ref:ts-plotdata2)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot the ts
plot(datI, main="", yax.flip=TRUE)
...
## Decomposition of time series {#sec-ts-decomposition-of-time-series}
...
### Estimating trends {#sec-ts-estimating-trends}
...
```{r ts-makeFilter, eval=TRUE, echo=TRUE}
...
```{r ts-plotTrendTSa, eval=FALSE, echo=TRUE}
## estimate of trend
...
\noindent The trend is a more-or-less smoothly increasing function over time, the average slope of which does indeed appear to be increasing over time as well (Figure \@ref(fig:ts-plotTrendTSb)).


(ref:ts-plotTrendTSb) Time series of the estimated trend $\{\hat{m}_t\}$ for the atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i.

```{r ts-plotTrendTSb, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotTrendTSb)'}
## estimate of trend
co2.trend <- filter(co2, filter=fltr, method="convo", sides=2)
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
...
### Estimating seasonal effects {#sec-ts-estimating-seasonal-effects}
...
```{r ts-getSeason, eval=TRUE, echo=TRUE}
...
```{r ts-plotSeasTSa, eval=FALSE, echo=TRUE, fig.show='hide'}
## plot the monthly seasonal effects
plot.ts(co2.1T, ylab="Seasonal effect", xlab="Month", cex=1)
```


(ref:ts-plotSeasTSb) Time series of seasonal effects plus random errors for the atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i, measured monthly from March 1958 to present.

```{r ts-plotSeasTSb, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotSeasTSb)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot the ts
...
```{r ts-getSeasonTS, eval=TRUE, echo=TRUE}
## length of ts
ll <- length(co2.1T)
## frequency (ie, 12)
ff <- frequency(co2.1T)
...
```{r ts-plotdataPar3, eval=FALSE, echo=TRUE, fig.show='hide'}
## plot the monthly seasonal effects
plot.ts(mm, ylab="Seasonal effect", xlab="Month", cex=1)
```

\noindent It looks like, on average, that the CO$_2$ concentration is highest in spring (March) and lowest in summer (August) (Figure \@ref(fig:ts-plotSeasMean)).  (*Aside*: Do you know why this is?)


(ref:ts-plotSeasMean) Estimated monthly seasonal effects for the atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i.

```{r ts-plotSeasMean, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotSeasMean)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot the ts
plot.ts(mm, ylab="Seasonal effect", xlab="Month", cex=1)
...
```{r ts-getSeasonMean, eval=TRUE, echo=TRUE}
...
### Completing the model {#sec-ts-completing-the-model}
...
```{r ts-getError, eval=TRUE, echo=TRUE}
## random errors over time
co2.err <- co2 - co2.trend - co2.seas
```
  
Now that we have all 3 of our model components, let's plot them together with the observed data $\{x_t\}$.  The results are shown in Figure \@ref(fig:ts-plotTrSeas).

```{r ts-plotdataPar4, eval=FALSE, echo=TRUE, fig.show='hide'}
## plot the obs ts, trend & seasonal effect
plot(cbind(co2,co2.trend,co2.seas,co2.err),main="",yax.flip=TRUE)
```


(ref:ts-plotTrSeas) Time series of the observed atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i (top) along with the estimated trend, seasonal effects, and random errors.

```{r ts-plotTrSeas, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=6, fig.cap='(ref:ts-plotTrSeas)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot the ts
plot(cbind(co2,co2.trend,co2.seas,co2.err), main="", yax.flip=TRUE)
...
### Using ```decompose()``` for decomposition {#sec-ts-using-verbdecompose-for-decomposition}
...
```{r ts-decompCO2, eval=TRUE, echo=TRUE}
...
We can easily make plots of the output and compare them to those in Figure \@ref(fig:ts-plotTrSeas):

```{r ts-plotDecompA, eval=FALSE, echo=TRUE, fig.show='hide'}
## plot the obs ts, trend & seasonal effect
plot(co2.decomp, yax.flip=TRUE)
```


(ref:ts-plotDecompB) Time series of the observed atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i (top) along with the estimated trend, seasonal effects, and random errors obtained with the function ```decompose()```.

```{r ts-plotDecompB, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=6, fig.cap='(ref:ts-plotDecompB)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot the ts
plot(co2.decomp, yax.flip=TRUE)
...
\noindent The results obtained with ```decompose()``` (Figure \@ref(fig:ts-plotDecompB)) are identical to those we estimated previously.
...
## Differencing to remove a trend or seasonal effects {#sec-ts-differencing-to-remove-a-trend-or-seasonal-effects}
...
### Using the ```diff()``` function {#sec-ts-using-the-verbdiff-function}
...
Let's use ```diff()``` to remove the trend and seasonal signal from the CO$_2$ time series, beginning with the trend.  Close inspection of Figure \@ref(fig:ts-plotdata1) would suggest that there is a nonlinear increase in CO$_2$ concentration over time, so we'll set ```differences=2```):
  
```{r ts-plotCO2diff2Echo, eval=FALSE, echo=TRUE, fig.show='hide'}
...
```{r ts-plotCO2diff2eval, eval=TRUE, echo=FALSE}
## twice-difference the CO2 data
co2.D2 <- diff(co2, differences=2)
```
  

(ref:ts-plotCO2diff2) Time series of the twice-differenced atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i.

```{r ts-plotCO2diff2, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotCO2diff2)'}
## set the margins & text size
...
We were apparently successful in removing the trend, but the seasonal effect still appears obvious (Figure \@ref(fig:ts-plotCO2diff2)).  Therefore, let's go ahead and difference that series at lag-12 because our data were collected monthly.

```{r ts-plotCO2diff12Echo, eval=FALSE, echo=TRUE}
...
```{r ts-plotCO2diff12eval, eval=TRUE, echo=FALSE}
## difference the differenced CO2 data
co2.D2D12 <- diff(co2.D2, lag=12)
```
  

(ref:ts-plotCO2diff12) Time series of the lag-12 difference of the twice-differenced atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i.

```{r ts-plotCO2diff12, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotCO2diff12)'}
## set the margins & text size
par(mar=c(4,4.5,1,1), oma=c(0,0,0,0), cex=1)
## plot the newly differenced data
...
\noindent Now we have a time series that appears to be random errors without any obvious trend or seasonal components (Figure \@ref(fig:ts-plotCO2diff12)).


## Correlation within and among time series {#sec-ts-correlation-within-and-among-time-series}
...
### Autocorrelation function (ACF) {#sec-ts-autocorrelation-function-acf}
...
```{r ts-plotACFa, eval=FALSE, echo=TRUE}
## correlogram of the CO2 data
acf(co2, lag.max=36)
```


(ref:ts-plotACFb) Correlogram of the observed atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i obtained with the function ```acf()```.

```{r ts-plotACFb, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotACFb)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## correlogram of the CO2 data
acf(co2, lag.max=36)
...
There are 4 things about Figure \@ref(fig:ts-plotACFb) that are noteworthy:
...
```{r ts-BetterPlotACF, eval=TRUE, echo=TRUE}
## better ACF plot
...
```{r ts-betterACF, eval=FALSE, echo=TRUE}
## acf of the CO2 data
...
```{r ts-DoOurACF, eval=TRUE, echo=FALSE}
## acf of the CO2 data
co2.acf <- acf(co2, lag.max=36)
```


(ref:ts-plotbetterACF) Correlogram of the observed atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i obtained with the function ```plot.acf()```.

```{r ts-plotbetterACF, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotbetterACF)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## correlogram of the CO2 data
plot.acf(co2.acf)
```
Notice that all of the relevant information is still there (Figure \@ref(fig:ts-plotbetterACF)), but now $r_0=1$ is not plotted at lag-0 and the lags on the $x$-axis are displayed correctly as integers.
...
```{r ts-LinearACFecho, eval=FALSE, echo=TRUE}
## length of ts
nn <- 100
...
```{r ts-LinearACF, eval=TRUE, echo=FALSE}
## length of ts
nn <- 100
## create straight line
tt <- seq(nn)
...
(ref:ts-plotLinearACF) Time series plot of a straight line (left) and the correlogram of its ACF (right).

```{r ts-plotLinearACF, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotLinearACF)'}
## set the margins & text size
...
plot.acf(line.acf)
```
The correlogram for a straight line is itself a linearly decreasing function over time (Figure \@ref(fig:ts-plotLinearACF)).

Now let's examine the ACF for a sine wave and see what sort of pattern arises.

```{r ts-SineACFecho, eval=FALSE, echo=TRUE}
## create sine wave
...
```{r ts-SineACF, eval=TRUE, echo=FALSE}
## create sine wave
tt <- sin(2*pi*seq(nn)/12)
## get ACF
sine.acf <- acf(tt, plot=FALSE)
...
(ref:ts-plotSineACF) Time series plot of a discrete sine wave (left) and the correlogram of its ACF (right).

```{r ts-plotSineACF, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotSineACF)'}
## set the margins & text size
par(mfrow=c(1,2), mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot line
plot.ts(tt, ylab=expression(italic(x[t])))
...
plot.acf(sine.acf)
```
Perhaps not surprisingly, the correlogram for a sine wave is itself a sine wave whose amplitude decreases linearly over time (Figure \@ref(fig:ts-plotSineACF)).

Now let's examine the ACF for a sine wave with a linear downward trend and see what sort of patterns arise.

```{r ts-SiLineACFecho, eval=FALSE, echo=TRUE}
...
```{r ts-SiLiACF, eval=TRUE, echo=FALSE}
## create sine wave with trend
tt <- sin(2*pi*seq(nn)/12) - seq(nn)/50
## get ACF
sili.acf <- acf(tt, plot=FALSE)
...
(ref:ts-plotSiLiACF) Time series plot of a discrete sine wave (left) and the correlogram of its ACF (right).

```{r ts-plotSiLiACF, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotSiLiACF)'}
## set the margins & text size
par(mfrow=c(1,2), mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot line
plot.ts(tt, ylab=expression(italic(x[t])))
...
plot.acf(sili.acf)
```
The correlogram for a sine wave with a trend is itself a nonsymmetrical sine wave whose amplitude and center decrease over time (Figure \@ref(fig:ts-plotSiLiACF)).
...
### Partial autocorrelation function (PACF) {#sec-ts-partial-autocorrelation-function-pacf}
...
```{r ts-plotPACFa, eval=FALSE, echo=TRUE}
...
```{r ts-BetterPlotPACF, eval=TRUE, echo=TRUE}
## better PACF plot
...
(ref:ts-plotPACFb) Correlogram of the PACF for the observed atmospheric CO$_2$ concentration at Mauna Loa, Hawai'i obtained with the function ```pacf()```.

```{r ts-plotPACFb, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotPACFb)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## correlogram of the CO2 data
pacf(co2, lag.max=36)
```
Notice in Figure \@ref(fig:ts-plotPACFb) that the partial autocorrelation at lag-1 is very high (it equals the ACF at lag-1), but the other values at lags > 1 are relatively small, unlike what we saw for the ACF.  We will discuss this in more detail later on in this lab.
...
```{r ts-CO2PACFecho, eval=FALSE, echo=TRUE}
## PACF of the CO2 data
co2.pacf <- pacf(co2)
## correlogram of the CO2 data
plot.acf(co2.pacf)
...
### Cross-correlation function (CCF) {#sec-ts-cross-correlation-function-ccf}
...
```{r ts-LynxSunspotCCF, eval=TRUE, echo=TRUE}
...
```{r ts-plotSunsLynxEcho, eval=FALSE, echo=TRUE}
## plot time series
plot(cbind(suns,lynx), yax.flip=TRUE)
```


(ref:ts-plotSunsLynx) Time series of sunspot activity (top) and lynx trappings in Canada (bottom) from 1821-1934.

```{r ts-plotSunsLynx, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=6, fig.cap='(ref:ts-plotSunsLynx)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot the ts
...
```{r ts-plotCCFa, eval=FALSE, echo=TRUE}
## CCF of sunspots and lynx
ccf(suns, log(lynx), ylab="Cross-correlation")
```


(ref:ts-plotCCFb) CCF for annual sunspot activity and the log of the number of lynx trappings in Canada from 1821-1934.

```{r ts-plotCCFb, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotCCFb)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## CCF of sunspots and lynx
...
From Figures \@ref(fig:ts-plotSunsLynx) and \@ref(fig:ts-plotCCFb) it looks like lynx numbers are relatively low 3-5 years after high sunspot activity (*i.e.*, significant correlation at lags of -3 to -5).


## White noise (WN) {#sec-ts-white-noise-wn}
...
### Simulating white noise {#sec-ts-simulating-white-noise}
...
```{r ts-DWNsim, echo=TRUE, eval=TRUE}
set.seed(123)
...
```{r ts-DWNsimPlotEcho, echo=TRUE, eval=FALSE}
## set up plot region
par(mfrow=c(1,2))
...
(ref:ts-plotDWNsims) Time series plots of simulated Gaussian (left) and Poisson (right) white noise.

```{r ts-plotDWNsims, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotDWNsims)'}
## set the margins & text size
...
```{r ts-DWNacfEcho, echo=TRUE, eval=FALSE}
## set up plot region
par(mfrow=c(1,2))
## plot normal variates with mean
...
(ref:ts-plotACFdwn) ACF's for the simulated Gaussian (left) and Poisson (right) white noise shown in Figure \@ref(fig:ts-plotDWNsims).

```{r ts-plotACFdwn, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotACFdwn)'}
## set the margins & text size
par(mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1, mfrow=c(1,2))
## plot normal variates with mean
acf(GWN, main="", lag.max=20)
...
## Random walks (RW) {#sec-ts-random-walks-rw}
...
### Simulating a random walk {#sec-ts-simulating-a-random-walk}
...
```{r ts-RWsim, eval=TRUE, echo=TRUE}
...
```{r ts-plotRWecho, eval=FALSE, echo=TRUE}
## setup plot area
par(mfrow=c(1,2))
## plot line
...
```{r ts-calcRWACF, eval=TRUE, echo=FALSE}
xx.acf <- acf(xx, plot=FALSE)
```


(ref:ts-plotRW) Simulated time series of a random walk model (left) and its associated ACF (right).

```{r ts-plotRW, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotRW)'}
## setup plot area
par(mfrow=c(1,2), mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot line
plot.ts(xx, ylab=expression(italic(x[t])))
...
\noindent Perhaps not surprisingly based on their names, autoregressive models such as RW's have a high degree of autocorrelation out to long lags (Figure \@ref(fig:ts-plotRW)).


### Alternative formulation of a random walk {#sec-ts-alternative-formulation-of-a-random-walk}
...
```{r ts-RWsimAlt, eval=TRUE, echo=TRUE}
## simulate RW
x2 <- cumsum(ww)
```
...
```{r ts-plotRWsimEcho, eval=FALSE, echo=TRUE}
## setup plot area
par(mfrow=c(1,2))
## plot 1st RW
plot.ts(xx, ylab=expression(italic(x[t])))
...
(ref:ts-plotRWalt) Time series of the same random walk model formulated as Equation \@ref(eq:defnRW2) and simulated via a for loop (left), and as Equation \@ref(eq:defnRWalt3) and simulated via ```cumsum()``` (right).

```{r ts-plotRWalt, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotRWalt)'}
## setup plot area
par(mfrow=c(1,2), mar=c(4,4,1,1), oma=c(0,0,0,0), cex=1)
## plot 1st RW
plot.ts(xx, ylab=expression(italic(x[t])))
...
## Autoregressive (AR) models {#sec-ts-autoregressive-ar-models}
...
### Simulating an AR($p$) process {#sec-ts-simulating-an-arp-process}
...
```{r ts-simAR1, echo=TRUE, eval=TRUE}
set.seed(456)
...
```{r ts-plotAR1sims, eval=FALSE, echo=TRUE}
## setup plot region
par(mfrow=c(1,2))
...
```{r ts-getPlotLims, eval=TRUE, echo=FALSE}
## get y-limits for common plots
ylm <- c(min(AR1.sm,AR1.lg), max(AR1.sm,AR1.lg))
```


(ref:ts-plotAR1contrast) Time series of simulated AR(1) processes with $\phi=0.1$ (left) and $\phi=0.9$ (right).

```{r ts-plotAR1contrast, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotAR1contrast)'}
## set the margins & text size
...
What do you notice about the two plots in Figure \@ref(fig:ts-plotAR1contrast)?  It looks like the time series with the smaller AR coefficient is more "choppy" and seems to stay closer to 0 whereas the time series with the larger AR coefficient appears to wander around more.  Remember that as the coefficient in an AR(1) model goes to 0, the model approaches a WN sequence, which is stationary in both the mean and variance.  As the coefficient goes to 1, however, the model approaches a random walk, which is not stationary in either the mean or variance.
...
```{r ts-simAR1opps, echo=TRUE, eval=TRUE}
set.seed(123)
## list description for AR(1) model with small coef
...
```{r ts-plotAR1oppsEcho, eval=FALSE, echo=TRUE}
## setup plot region
par(mfrow=c(1,2))
## get y-limits for common plots
...
```{r ts-getPlotLimsOpps, eval=TRUE, echo=FALSE}
## get y-limits for common plots
ylm <- c(min(AR1.pos,AR1.neg), max(AR1.pos,AR1.neg))
```


(ref:ts-plotAR1opps) Time series of simulated AR(1) processes with $\phi_1=0.5$ (left) and $\phi_1=-0.5$ (right).

```{r ts-plotAR1opps, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotAR1opps)'}
## set the margins & text size
par(mfrow=c(1,2), mar=c(4,4,1.5,1), oma=c(0,0,0,0), cex=1)
## plot the ts
plot.ts(AR1.pos, ylim=ylm,
...
```{r ts-ARpFail, eval=FALSE, echo=TRUE}
...
### Correlation structure of AR($p$) processes {#sec-ts-correlation-structure-of-arp-processes}
...
```{r ts-ARpSims, eval=TRUE, echo=TRUE}
set.seed(123)
...
```{r ts-plotARpCompsEcho, eval=FALSE, echo=TRUE}
## set up plot region
par(mfrow=c(4,3))
## loop over orders of p
for(p in 1:4) {
...
(ref:ts-plotARpComps) Time series of simulated AR($p$) processes (left column) of increasing orders from 1-4 (rows) with their associated ACF's (center column) and PACF's (right column).  Note that only the first 50 values of $x_t$ are plotted.

```{r ts-plotARpComps, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=8, fig.cap='(ref:ts-plotARpComps)'}
## set the margins & text size
...
\noindent As we saw in lecture and is evident from our examples shown in Figure \@ref(fig:ts-plotARpComps), the ACF for an AR($p$) process tails off toward zero very slowly, but the PACF goes to zero for lags > $p$.  This is an important diagnostic tool when trying to identify the order of $p$ in ARMA($p,q$) models.


## Moving-average (MA) models {#sec-ts-moving-average-ma-models}
...
### Simulating an MA($q$) process {#sec-ts-simulating-an-maq-process}
...
```{r ts-simMA1opps, echo=TRUE, eval=TRUE}
set.seed(123)
...
```{r ts-plotMA1oppsEcho, eval=FALSE, echo=TRUE}
## setup plot region
par(mfrow=c(1,3))
## plot the ts
plot.ts(MA1.sm,
...
(ref:ts-plotMA1opps) Time series of simulated MA(1) processes with $\theta=0.2$ (left), $\theta=0.8$ (middle), and $\theta=-0.5$ (right).

```{r ts-plotMA1opps, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=3, fig.cap='(ref:ts-plotMA1opps)'}
## set the margins & text size
...
### Correlation structure of MA($q$) processes {#sec-ts-correlation-structure-of-maq-processes}
...
```{r ts-MAqSims, eval=TRUE, echo=TRUE}
set.seed(123)
...
```{r ts-plotMApCompsEcho, eval=FALSE, echo=TRUE}
## set up plot region
par(mfrow=c(4,3))
## loop over orders of q
for(q in 1:4) {
...
(ref:ts-plotMApComps) Time series of simulated MA($q$) processes (left column) of increasing orders from 1-4 (rows) with their associated ACF's (center column) and PACF's (right column).  Note that only the first 50 values of $x_t$ are plotted.

```{r ts-plotMApComps, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=8, fig.cap='(ref:ts-plotMApComps)'}
## set the margins & text size
par(mfrow=c(4,3), mar=c(4,4,0.5,0.5), oma=c(0,0,0,0), cex=1)
## loop over orders of q
for(q in 1:4) {
...
\noindent Note very little qualitative difference in the realizations of the four MA($q$) processes (Figure \@ref(fig:ts-plotMApComps)).  As we saw in lecture and is evident from our examples here, however, the ACF for an MA($q$) process goes to zero for lags > $q$, but the PACF tails off toward zero very slowly.  This is an important diagnostic tool when trying to identify the order of $q$ in ARMA($p,q$) models.



## Autoregressive moving-average (ARMA) models {#sec-ts-autoregressive-moving-average-arma-models}
...
### Fitting ARMA($p,q$) models with ```arima()``` {#sec-ts-fitting-armapq-models-with-verbarima}
...
```{r ts-ARMAest, eval=TRUE, echo=TRUE}
set.seed(123)
...
### Searching over model orders {#sec-ts-searching-over-model-orders}
...
```{r ts-ARMAsearch1, eval=TRUE, echo=TRUE, cache=TRUE}
...
```{r ts-autoARIMA, eval=TRUE, echo=TRUE, cache=TRUE}
...
1. Convert ``pDat``, which is a **data.frame** object, into a **ts** object.  This bit of code might be useful to get you started:
    ```{r ts-HW1_1, eval=FALSE, echo=TRUE}
