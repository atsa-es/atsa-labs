...
After updating to the latest version of R, install JAGS for your operating platform using the instructions [here](http://sourceforge.net/projects/mcmc-jags/files/).  Click on JAGS, then the most recent folder, then the platform of your machine.  You will also need the **coda** and **R2jags** packages.
...
",file=model.loc)
```
```{r clean2, echo=FALSE, purl=FALSE}
removefiles=c(removefiles, model.loc)
```
