...
\end{align}
If we define $\ttheta_t = \begin{bmatrix}\alpha \\ \beta\end{bmatrix}_t$, $\GG_t = \II$, $\ww_t = \begin{bmatrix} w_\alpha \\ w_\beta\end{bmatrix}_t$, and $\QQ = \begin{bmatrix}q_\alpha& 0 \\ 0&q_\beta\end{bmatrix}$, we get Equation \@ref(eq:dlm-dlm2). If we define $y_t = survival_t$ and $\FF_t = \begin{bmatrix}1 \\ F_t\end{bmatrix}$, we can write out the full univariate DLM as a state-space model with the following form:
\begin{equation}
(\#eq:dlm-dlmSW3)
\begin{gathered}
