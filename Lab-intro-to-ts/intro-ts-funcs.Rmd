```{r ts-RUNFIRST, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
require(forecast)
require(RCurl)
require(stringr)
tabledir="../figures/"
...
# Basic time-series functions in R  {#chap-introTS}
...
Let's start by importing some data; the record of the atmospheric concentration of CO$_2$ collected at the Mauna Loa Observatory in Hawai'i makes a nice example.  The data file contains some extra information that we don't need, so we'll only read in a subset of the columns (*i.e.*, 1, 2 \& 5).

```{r ts-CO2data, echo=TRUE, eval=TRUE}
...
Before we examine the CO$_2$ data further, however, let's see a quick example of how you can combine and plot multiple time series together. We'll begin by getting a second time series (monthly mean temperature anomolies for the Northern Hemisphere) and convert them to a **ts** object.

```{r ts-Temp_data, echo=TRUE, eval=TRUE}
...
```
```{r ts-Temp_data_offline, echo=FALSE, eval=FALSE}
