...
This chapter will show you how to fit some basic univariate state-space models using the **MARSS** package. This will also introduce you to the idea of writing AR models in state-space form.  
...
All the data used in the chapter are in the **MARSS** package. The other  required packages are **stats** (normally loaded by default when starting R), **datasets** and **forecast**.  To run the JAGS code example (optional), you will also need [JAGS](http://mcmc-jags.sourceforge.net/) installed and the **R2jags** and **coda** R packages.  Install the packages, if needed, and load:
...
```{r set-set-invisible, echo=FALSE}
set.seed(123)
```
...
```{r uss-jags-fit, results='hide', message=FALSE}
...
```{r uss-fig-posteriors, fig=TRUE, fig.cap='(ref:uss-fig-posteriors)', message=FALSE}
attach.jags(mod_ss)
par(mfrow=c(2,2))
hist(mu)
