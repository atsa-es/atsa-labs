...
A script with all the R code in the chapter can be downloaded  [here]("./Rcode/fitting-univariate-state-space.R"). 

### Data and packages  {-}

All the data used in the chapter are in the **MARSS** package. The other  required packages are **stats** (normally loaded by default when starting R), **datasets** and **forecast**.  To run the JAGS code example, you will also need [JAGS](http://mcmc-jags.sourceforge.net/) installed and the **R2jags** and **coda** R packages.  Install the packages, if needed, and load:
...
*Important: this state-space model, $\yy$ is always the data and $\xx$ is a hidden random walk estimated from the data.*
...
If we wanted to fix $q=0.1$, then $\QQ=[0.1]$ (a $1 \times 1$ matrix with 0.1). We just change ```mod.list$Q``` and re-fit:
...
## Fitting a univariate AR-1 state-space model with JAGS* {#sec-uss-fitting-a-univariate-ar--state-space-model-with-jags}

Here we show how to fit model 3, Equation \@ref(eq:uss-random-walk-w-noise-w-drift), with JAGS.  This section requires that you have JAGS installed and the **R2jags** and **coda** R packages loaded.
...
```{r uss-fig-posteriors, echo=TRUE, fig=TRUE, fig.cap='(ref:uss-fig-posteriors)'}
attach.jags(mod_ss)
par(mfrow=c(2,2))
hist(mu)
...
```{r uss-fig-bayesian-states, echo=TRUE, fig=TRUE, fig.cap='(ref:uss-fig-bayesian-states)'}
...
## A simple random walk model of animal movement {#sec-uss-a-simple-random-walk-model-of-animal-movement}
...
```
Use ```?rnorm``` to make sure you know what the arguments to the ```rnorm()``` function are.
...
a.  How are the two estimates different?
...
2.  What is the $\xx$ model for ```diff.dat```?  Look at your answer to part (a) and the answer to part (e).
...
7. Evaluate the predictive accuracy of forecasts using the **forecast** package using the ```airmiles``` dataset.
