...
For these problems, use the following code to load in phytoplankton data, covariates, and z-score all the data.  For the covariates, you'll use 5-month lagged phosphorous and 1-month lagged temperature.  Then use `dat` and `covars` directly in your code.
...
dat <- (dat-avg)/sd
rownames(dat)=phytos
#lag the temp by one month and TP by 5 months
yrs1 <- which(lakeWAplanktonTrans[,"Year"]==1984&lakeWAplanktonTrans[,"Month"]==12)
yrs2 <- which(lakeWAplanktonTrans[,"Year"]==1994&lakeWAplanktonTrans[,"Month"]==11)
yrs3 <- which(lakeWAplanktonTrans[,"Year"]==1984&lakeWAplanktonTrans[,"Month"]==8)
yrs4 <- which(lakeWAplanktonTrans[,"Year"]==1994&lakeWAplanktonTrans[,"Month"]==7)
#z-score the covariates
df<-data.frame(Temp=lakeWAplanktonTrans[yrs1:yrs2,"Temp"], Month=rep(month.abb,10))
dc.temp <- residuals(lm(Temp~Month, data=df))
df<-data.frame(TP=lakeWAplanktonTrans[yrs1:yrs2,"TP"], Month=rep(month.abb,10))
dc.tp <- residuals(lm(TP~Month, data=df))
covars <- rbind(Temp=dc.temp, TP=dc.tp)
avg <- apply(covars, 1, mean)
sd <- sqrt(apply(covars, 1, var, na.rm=TRUE))
covars <- (covars-avg)/sd
covars <- rbind(covars, covars[1,]*covars[2,])
rownames(covars) <- c("Temp","TP","Inter")
...
1. How does month affect the mean phytoplankton population growth rates? Show a plot of mean growth rate versus month for each taxon. Estimate seasonal effects without any covariate (Temp, TP) effects.

2. It is likely that both temperature and total phosphorus (TP) affect phytoplankton population growth rates. Using MARSS models, evaluate which is the more important driver or if both are important. 


Leave out the seasonal covariates from question 1, i.e. only use Temp and TP as covariates.
