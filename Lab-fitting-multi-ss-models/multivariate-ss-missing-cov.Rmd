...
For the first analysis, we are just going to look at February Snow Water Equivalent (SWE).  Our subset of stations is `y$Station.Id`.  There are many missing years among some of our stations.
...
fit.ar1 <- MARSS(dat, model=mod.list.ar1, control=list(maxit=5000), inits=list(A=matrix(m,ns,1)))
```
...
m <- apply(dat, 1, mean, na.rm=TRUE)
fit.corr <- MARSS(dat, model=mod.list.corr, control=list(maxit=5000), inits=list(A=matrix(m,ns,1)))
```


The estimated SWEs for the missing years uses the information about the correlation with other sites only.  

```{r mssmiss-snotelplotstates2, include=FALSE}
d <- tidy(fit2, type="states")
d$Year <- d$t + 1980
d$Station <- stringr::str_replace(d$term,"X.","")
p <- ggplot(data = d) + 
  geom_line(aes(Year, estimate)) +
...
library(MARSS)
m <- apply(dat, 1, mean, na.rm=TRUE)
fit.dfa <- MARSS(dat, model=mod.list.dfa, control=list(maxit=1000), inits=list(A=matrix(m,ns,1)))
```
...
dat$Station <- droplevels(dat$Station)
...
```{r mssmiss-month-factor-marss-params}
ns <- dim(dat)[1]
B <- "diagonal and equal"
Q <- "unconstrained"
U <- "zero"
...
c <- c.seas
x0 <- "zero"
mod.list.seas <- list(B=B, U=U, Q=Q, A=A, R=R, C=C, c=c, x0=x0, tinitx=0)
```

```{r mssmiss-seas-ar1, results="hide"}
m <- apply(dat, 1, mean, na.rm=TRUE)
fit.seas <- MARSS(dat, model=mod.list.seas, control=list(maxit=500), inits=list(A=matrix(m,ns,1)))
```
...
fit <- fit.seas
d <- augment(fit, interval="confidence")
d$Year <- rep(rep(1981:2013, each=12),ns)
