...
```{r mssmiss-loaddata, include=FALSE, eval=FALSE}
...
The $b$ estimate is `r coef(fit.ar1)`.
...
```{r mssmiss-stateresids-plot-fit-ar1, warning=FALSE, results='hide'}
fit <- fit.ar1
...
```{r mssmiss-stateresids-plot-fit-corr, warning=FALSE, results='hide'}
fit <- fit.corr
par(mfrow=c(4,4),mar=c(2,2,1,1))
apply(residuals(fit)$state.residuals[,1:30], 1, acf)
```
