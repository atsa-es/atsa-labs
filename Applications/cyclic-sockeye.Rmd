...
Then we make our model list. We need to set $\AA$ since `MARSS()` doesn't like the default value of `scaling` when $\ZZ$ is time-varying.
...
* Each trend $x$ will be modeled as separate but allowed to be correlated. This means either an unconstrained $\QQ$ or an equal variance and equal covariance matrix.
* Each seasonal trend, the $\beta$'s, will also be treated as separate but independent. This means either a diagonal and equal variance $\QQ$ or diagona and unequal variances.

The $\xx$ equation is then:
...
The following code will create the $\ZZ$ for a model with $n$ rivers. The first $\ZZ$ is shown.
...
And this code will make the $\QQ$ matrix:
