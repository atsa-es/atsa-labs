...
knitr::opts_knit$set(unnamed.chunk.label = "pva-")
...
\begin{align}
x_t = x_{t-1}+u + w_t 	& \qquad\textup{where}\; w_t \sim \text{N}(0, \sigma^2)	(\#eq:1a)\\
y_t = x_t + v_t 	& \qquad\textup{where}\; v_t \sim \text{N}(0, \eta^2) (\#eq:1b)
\end{align}
...
```
 ...
 Stochastic population trajectories show much variation, so it is best to look at a few simulated data sets at once.  In Figure \@ref(fig:pva-9sim), nine simulations from the identical parameters are shown.

(ref:pva-9sim) Plot of nine simulated population time series with process and observation error.  Circles are observation and the dashed line is the true population size.

```{r pva-9sim, echo=FALSE, fig.cap='(ref:pva-9sim)'}
par(mfrow=c(3,3))
sim.u = -0.05        # growth rate
sim.Q = 0.02         # process error variance
sim.R = 0.05         # non-process error variance
...
### The effect of parameter values on parameter estimates {#sec-pva-ex1}
...
\index{estimation!EM}\index{estimation!maximum-likelihood}Using the simulated  data, we estimate the parameters, $u$, $\sigma^2$, and $\eta^2$, and the hidden population sizes.   These are the estimates using a model with process and observation variability.  We will use the **MARSS** package to fit the model.  The function call is `kem = MARSS(data)`, where `data` is a vector of logged (base e) counts with missing values denoted by NA.  After this call, the maximum-likelihood parameter estimates are shown with `coef(kem)`.  There are numerous other outputs from the `MARSS()` function.  To get a list of the standard model output available type in `?print.MARSS`.  Note that `kem` is just a name; the output could have been called `foo`.  Here's code to fit to the simulated time series:
```{r pva-fitKalmanEM1, eval=FALSE}
library(MARSS)
kem = MARSS(y) 
```

```{r pva-fitKalmanEM2, echo=FALSE}
library(MARSS)
...
The MARSS model fit also gives an estimate of the true population size with observation error removed.  This is in `kem$states`. Figure \@ref(fig:pva-plotkemstates) shows the estimated true states of the population over time as a solid line. Note that the solid line is considerably closer to the actual true states (dashed line) than the observations. On the other hand with certain datasets, the estimates can be quite wrong as well! 

(ref:pva-plotkemstates) The circles are the observed population sizes with error.  The dashed lines are the true population sizes.  The solid thin lines are the estimates of the true population size from the MARSS model.  When the process error variance is 0, these lines are straight.

```{r pva-plotkemstates, echo=FALSE, fig.cap='(ref:pva-plotkemstates)'}
par(mfrow=c(3,3))
for(i in 1:9){
plot(years, y.tss[i,], xlab="",ylab="index of log abundance",lwd=2,bty="l")
lines(years,x.tss[i,],type="l",lwd=2,lty=2)
...
```
Next, we regress the changes in population size between censuses (`delta.pop`) on the time between censuses (`tau`) while setting the regression intercept to 0.  The slope of the resulting regression line is an estimate of $u$, while the variance of the residuals around the line is an estimate of $\sigma^2$. The regression is shown in Figure \@ref(fig:pva-dennisregression).  Here is the code to do that regression:
```{r dennisEsts}
...
(ref:pva-dennisregression) The regression of $log(N_{t+\tau})-log(N_t)$ against $\tau$.  The slope is the estimate of $u$ and the variance of the residuals is the estimate of $\sigma^2$. The regression is constrained to go through (0,0).

```{r pva-dennisregression, echo=FALSE, fig.cap='(ref:pva-dennisregression)'}
par(mfrow=c(1,1))
plot(tau, delta.pop,xlab="time step size (tau)",ylab="population transition size",xlim=c(0,max(tau)),bty="l")
abline(den91,col=2)
```

Here are the parameter values for the data in Figure \@ref(fig:pva-plotkemstates) using the process-error only model:
```{r pva-paramvals, echo=FALSE}
...
for(i in 1:9){
	...
	delta.pop <- diff(den.yts) # store each transition
	tau <- diff(den.years)
...
library(kableExtra)
knitr::kable(den91.params, digits=3) %>%
...
### The variability in parameter estimates {#sec-pva-ex2}
...
```{r pva-Cs1-Exercise2, results='hide'}
sim.u <- -0.05   # growth rate
sim.Q <- 0.02    # process error variance
sim.R <- 0.05    # non-process error variance
nYr <- 50        # number of years of data to generate
...
for(i in 1:nsim){ 
  x.ts[i,1] <- init
  y.ts[i,1] <- x.ts[i,1]+rnorm(1,mean=0,sd=sqrt(sim.R))
  for(t in 2:nYr){	
  ...
  params[i,c("kem.U","kem.Q","kem.R")] = coef(kem,type="vector")[c(2,3,1)]
  ...
  delta.pop <- diff(den.yts) # transitions
  tau <- diff(den.years)    # time step length
...
Here is an example of the output from the Example \@ref(sec-pva-ex2) code:
```{r pva-Ex2-results, echo=FALSE}
library(kableExtra)
knitr::kable(params, digits=3) %>%
  kableExtra::kable_styling(full_width = FALSE)
```
...
(ref:pva-CS1-9probext) Plot of the true and estimated probability of declining 90\% in different time horizons for nine simulated population time series with observation error. The plot may look like a step-function if the $\sigma^2$ estimate is very small (<1e-4 or so).

```{r pva-CS1-9probext, echo=FALSE, fig.cap='(ref:pva-CS1-9probext)'}
...
for(j in c(10,1:8)){
	...
	u <- params[j,1]
	Q <- params[j,3]
  if(Q==0) Q <- 1e-4  #so the extinction calc doesn't choke
  p.ever <- ifelse(u<=0,1,exp(-2*u*xd/Q)) 
  if(is.finite(exp(2*xd*abs(u)/Q))){ #Q!=0
    part2 <- exp(2*xd*abs(u)/Q)*pnorm((-xd-abs(u)* tyrs)/sqrt(Q*tyrs))
  }else part2=0  #Q=0    
  kal.ex <- p.ever*pnorm((-xd+abs(u)*tyrs)/sqrt(Q*tyrs)) + part2

  #Dennis et al 1991
  u <- params[j,2]
  Q <- params[j,5]
  p.ever <- ifelse(u<=0,1,exp(-2*u*xd/Q)) 
  denn.ex <- p.ever*pnorm(( -xd + abs(u)*tyrs)/ (sqrt(Q)*sqrt(tyrs))) + exp(2*xd*abs(u)/Q) * pnorm((-xd - abs(u)* tyrs) / (sqrt(Q)*sqrt(tyrs)))

	#True
	u <- sim.u; Q <- sim.Q
  p.ever <- ifelse(u<=0,1,exp(-2*u*xd/Q)) 
	real.ex <- p.ever*pnorm(( -xd + abs(u)*tyrs )/ sqrt(Q*tyrs)) + exp(2*xd*abs(u)/Q) * pnorm((-xd - abs(u)* tyrs) / sqrt(Q*tyrs))
...
### The effect of parameter values on risk estimates {#sec-pva-ex3}

In this example, you will recreate Figure \@ref(fig:pva-CS1-9probext) using different parameter values.  This will give you a feel for how variability in the data and population process affect the risk estimates.  You'll need to run the Example \@ref(sec-pva-ex2) code before running the Example \@ref(sec-pva-ex3) code.

Begin by changing `sim.R` and rerunning the Example \@ref(sec-pva-ex2) code.  Now run the Example \@ref(sec-pva-ex3) code and generate parameter estimates.  When are the estimates using the process-error only model (`den91`) worse and in what way are they worse? You might imagine that you should always use a model that includes observation error, since in practice observations are never perfect.  However, there is a cost to estimating that extra variance parameter and the cost is a more variable $\sigma^2$ (`Q`) estimate.  Play with shortening the time series and decreasing the `sim.R` values.  Are there situations when the `cost' of the extra parameter is greater than the `cost' of ignoring observation error?

Next change the rate of decline in the simulated data.  To do this, rerun the Example \@ref(sec-CS1-ex2) code using a lower `sim.u`; then run the Example \@ref(sec-CS1-ex3) code. Do the estimates seem better or worse for rapidly declining populations?  Rerun the Example \@ref(sec-CS1-ex2) code using fewer number of years (`nYr` smaller) and increase `fracmiss`.  Run the Example \@ref(sec-CS1-ex3) code again.  The graphs will start to look peculiar.  Why do you think it is doing that? Hint: look at the estimated parameters.

Last change the extinction threshold (`pd` in the Example \@ref(sec-CS1-ex3) code). How does changing the extinction threshold change the extinction probability curves? Do not remake the data, i.e. don't rerun the Example \@ref(sec-pva-ex2) code.


```{r pva-Exercise3, results='hide', fig.show='hide'}
#Needs Example 2 to be run first
par(mfrow=c(3,3))
...
for(j in c(10,1:8)){
  ...
  p.ever <- ifelse(u<=0,1,exp(-2*u*xd/Q)) 
  part2 <- exp(2*xd*abs(u)/Q)*pnorm((-xd-abs(u)*tyrs)/sqrt(Q*tyrs))
  denn.ex <- p.ever*pnorm((-xd+abs(u)*tyrs)/sqrt(Q*tyrs)) + part2

  #True parameter values
  u <- sim.u;   Q <- sim.Q
  p.ever <- ifelse(u<=0,1,exp(-2*u*xd/Q)) 
  part2 <- exp(2*xd*abs(u)/Q)*pnorm((-xd-abs(u)*tyrs)/sqrt(Q*tyrs))
  real.ex <- p.ever*pnorm((-xd+abs(u)*tyrs)/sqrt(Q*tyrs)) + part2
...
\label{CS1.ex4}
Use the code below to re-create Figure \@ref(fig:pva-figTMU) and get a feel for when risk estimates are more certain and when they are less certain.  `N` are the number of years of data, `u` is the mean population growth rate, and `s2p` is the process variance.

```{r pva-Exercise4, fig.show='hide', results='hide'}
...
If you have your data in a tab delimited file with a header, you could read it in using `file.choose()` to call up a directory browser. However, the **MARSS** package has the four example datasets for you to work with: `wilddogs`, `prairiechicken`, `grouse`, and `graywhales`.

```{r pva-Exercise5, results='hide', fig.show='hide'}
data(wilddogs, package="MARSS")
dat <- wilddogs
CSEGriskfigure(dat, CI.method="hessian", silent=TRUE)
```
...
*  For each bootstrapped parameter set, compute a set of extinction estimates (you use Equation \@ref(eq:probext) and code from Example \@ref(sec-CS1-ex3))
