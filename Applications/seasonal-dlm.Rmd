...
$$y_t = \begin{bmatrix}1& \beta_1 \sin(2\pi t/p) + \beta_2 \cos(2\pi t/p)\end{bmatrix} \begin{bmatrix}x_1 \\ x_2 \end{bmatrix}_t + v_t$$

```{r mod.list2}
Z <- array(list(1), dim=c(1,2,TT))
Z[1,2,] <- paste0("0 + ",sin(2*pi*(1:TT)/12),"*beta1 + ",cos(2*pi*(1:TT)/12),"*beta2")
```
...
In this simulation, it gets the level well and get the shape of the $z_t$ but underestimates the size of the $z_t$ (amplitude of cycles). There is not a lot of data on the amplitude of the cycles as $z_t$ changes quite quickly so this is not surprising.

```{r echo=FALSE}
df <- data.frame(t=1:TT, value=zt, type="true", var="zt")
df <- rbind(df, data.frame(t=1:TT, value=xt, type="true", var="xt"))
df <- rbind(df, data.frame(t=1:TT, value=fit$states[2,], type="estimate", var="zt"))
df <- rbind(df, data.frame(t=1:TT, value=fit$states[1,], type="estimate", var="xt"))
ggplot(df, aes(x=t, y=value, color=type)) + geom_line() + facet_wrap(~var, scales="free_y")
```
